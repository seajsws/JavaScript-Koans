...
test('assigning functions to variables', function() {
  ...
  equal(triple(4), __, 'what is triple 4?');
});
...
test('self invoking functions', function() {
  ...
  (function(pv) {
    ...
    equal(pv, __, 'what is the value of pv?');
    equal(typeof(secretValue), '__', 'is secretValue available in this context?');
    equal(typeof(publicValue), '__', 'is publicValue available in this context?');
  })(publicValue);
...
test('using call to invoke function', function() {
  ...
  equal(result, __, "what will the value of invokee's this be?");
});
